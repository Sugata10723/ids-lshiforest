...
class KernelLSH(LSH):
	...
	def fit(self, data):
		...
		X = self._train_data = np.asmatrix(train_data)
...
#			elif type(self._kernel_kwds) == tp.StringType and self._kernel_kwds == 'range':
	...
	def _compute_kernel(self, X, Y):
		...
		part_X = sum(np.power(X.T, 2)).T * np.asmatrix(np.ones((1, n_Y)))
		part_Y = np.asmatrix(np.ones((n_X, 1))) * sum(np.power(Y.T, 2))
		part_XY = -2 * X * Y.T
	...
	def _generate_e_s(self):
		...
		return np.asmatrix(e_s)

	
	def compute_test_kernel(self, x):
		return self._compute_kernel(self._train_data, np.asmatrix(x))
	...
	def get_hash_value(self, test_kernel, hash_index):
		...
		test_kernel = np.asmatrix(test_kernel).T
