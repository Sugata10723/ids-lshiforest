...
class LSHTree:
	...
	def _recursive_build(self, data, depth_limit, lof=0, hash_func_index=0):
		...
		if n_samples == 1 or hash_func_index > depth_limit:
			...
			std = np.std(list(children_count.values())) # calc standard deviation
				
			children = {}
			for key in partition.keys():
				child_data = partition.get(key)
				children[key] = self._recursive_build(child_data, depth_limit, min(0.0, (children_count[key]-mean)/std if std != 0 else 0), cur_index+1)
	...
	def _split_data(self, data, depth):
		...
		for i in range(len(data)):
			key = self._lsh.get_hash_value(np.array(data[i][1:]), depth) # 行列固有の表記、1列目はインデックスなので使用しない
	...
	def predict(self, granularity, point):
		point = self._lsh.format_for_lsh(np.asmatrix(point)).A1
